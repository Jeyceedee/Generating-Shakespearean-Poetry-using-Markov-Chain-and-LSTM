...
def create_network(depth):
    ...
    model.compile(optimizer='rmsprop',
            loss='mse')
    if poet + ".poetry" in os.listdir(".") and train_mode == False:
        model.load_weights(str(poet + ".poetry"))
        print("loading saved network: " + str(poet) + ".poetry") 
    return model
...
def rhymeindex(lyrics):
    if str(poet) + ".rhymes" in os.listdir(".") and train_mode == False:
        print ("loading saved rhymes from " + str(poet) + ".rhymes")
        return open(str(poet) + ".rhymes", "r",encoding='utf-8').read().split("\n")
    else:
        rhyme_master_list = []
        ...
        print(rhymelist)
        f = open(str(poet) + ".rhymes", "w", encoding='utf-8')
        f.write("\n".join(rhymelist))
        f.close()
        return rhymelist
...
def compose_poetry(lines, rhyme_list, lyrics_file, model, numOfLines):
    poetry_vectors = []
    ...
    for i in range(numOfLines):
        poetry_vectors.append(model.predict(np.array([poetry_vectors[-1]]).flatten().reshape(1, 2, 2)))
...
def train(x_data, y_data, model):
    model.fit(np.array(x_data), np.array(y_data),
            ...
            epochs=5,
            verbose=1)
    model.save_weights(poet + ".poetry")
...
def main(depth, train_mode):
    ...
    if train_mode == False:
        vectors = compose_poetry(bars, rhyme_list, text_file, model, numOfLines)
        poetry = vectors_into_song(vectors, bars, rhyme_list)
        f = open(poetry_file, "w", encoding='utf-8')
        for bar in poetry:
            f.write(bar)
            f.write("\n")
    return('Shakespearean_poetry.txt')
